...
return function(mod)
  ...
  -- plain white either side of it.  A single BLACK pixel, centred in the
  -- cell, on a whole pixel like every other mark here (cursor stubs, the
  -- shiny diamond).  Black, not a mid-gray: PaletteFX only anchors shade 0
  -- (white) and shade 3 (black) near their DMG colors across every named
  -- palette -- shades 1 and 2 are real hues that vary per palette (MEWMON's
  -- shade 1 is salmon, data/palettes_gbc.lua), so a gray dot would come out
  -- some other color under most of the game's COLORS modes.  One pixel
  -- keeps it a hint at the grid's structure rather than a mark competing
  -- with the icons for attention, the way a larger black shape would.
  -- Called from the white stage of Screen:draw (see the comment there): it
  -- sets its own color and restores white so the icon draws around it stay
  -- correctly tinted.
  local EMPTY_DOT = 1
